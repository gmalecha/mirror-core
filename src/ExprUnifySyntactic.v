...
Section typed.
  Variable ts : types.
  Variable subst : Type.
  Variable Subst_subst : Subst ts subst. 
  Variable p : ProverT ts.
  Variable f : Facts p.
  
  Variable exprUnify : forall (n : nat) (s : subst) (l r : expr ts), option subst.

  Fixpoint exprUnify' (n : nat) (s : subst) (e1 e2 : expr ts) {struct e1} : option subst.
  refine (
    match e1 , e2 with
