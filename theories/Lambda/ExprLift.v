...
Section types.
  ...
  Definition lem_vars_to_uvars_exprD' : Prop :=
    ...
    exprD' nil tus (tvs ++ tvs') t e = Some val ->
    exists
      val' : hlist (typD nil) (tus ++ tvs') ->
             hlist (typD nil) tvs -> typD nil t,
      exprD' nil (tus ++ tvs') tvs t (vars_to_uvars (length tvs) (length tus) e) = Some val' /\
  ...
  Proof.
    ...
    { revert H; consider (v ?[ lt ] length tvs);
      ...
      { inv_all; subst.
        ...
        eapply nth_error_get_hlist_nth_rwR with (F := typD nil) in H3.
        revert H3.
        instantiate (1 := tvs').
