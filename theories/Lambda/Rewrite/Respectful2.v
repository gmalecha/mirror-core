...
Section setoid.
  ...
  Let typ := mtyp tsym.
  Context {func : Type}.
  Context {RType_typD : RType (typ 0)}. (** TODO: Could be fixed *)
  ...
  Context {RelDec_eq_typ : RelDec (@eq (typ 0))}.
  ...
  Definition expr_sdec : expr (typ 0) func -> expr (typ 0) func -> bool :=
    @expr_eq_sdec (typ 0) func _ func_sdec.

  Lemma expr_sdec_sound
  : forall a b : expr (typ 0) func, expr_sdec a b = true -> a = b.
  ...
  { relation : R
  ; term     : expr (typ 0) func
  }.

  (* TODO(mario): explain the difference between the following two functions *)
  Definition Proper_conclD (tus tvs : tenv (typ 0)) (p : Proper_concl)
  ...
  | PPr_tc : forall {n : list nat},
      polymorphic typ n Proper_concl ->
  ...
  Definition Pr' (r : R) (t : expr (typ 0) func) :=
    Pr (mkProper r t).

  (** polymorphic proper hint without typeclass constraints *)
  Definition PPr {n : list nat} (pc : polymorphic typ n Proper_concl) :=
    PPr_tc (n:=n) pc (tc_any n).

  Theorem PPr_sound {n : list nat} (pc : polymorphic typ n Proper_concl)
  ...
  Theorem PPr_tc_sound {n : list nat} (pc : polymorphic typ n Proper_concl) tc
  : polymorphicD (fun x => x) (with_typeclasses Proper_conclP tc pc) ->
  ...
  Local Definition Proper_lemma := Lemma.lemma (typ 0) (expr (typ 0) func) Proper_concl.
  ...
  Local Definition apply_respectful (lem : Proper_lemma)
             (tacK : rtacK (typ 0) (expr (typ 0) func))
  : ResolveProper _ _ _ :=
  ...
  Definition or_respectful
             (a b : expr (typ 0) func -> R -> mrw (typ 0) func (list R))
    : ResolveProper _ _ _ :=
  ...
  Definition fail_respectful : ResolveProper (typ 0) func Rbase :=
    fun _ _ => rw_fail.
  ...
  Local Definition do_one_prespectful (h : HintProper) : ResolveProper (typ 0) func Rbase :=
    match h with
    | PPr_tc pc tc =>
      (fun (e : expr (typ 0) func) =>
  ...
  Fixpoint do_prespectful (pdb : ProperDb) : ResolveProper (typ 0) func Rbase :=
    match pdb with
    | nil => fail_respectful
    | p :: pdb' =>
...
Section Reify_Proper_concl.
  ...
  Let Ty := mtyp tsym.
  Variables func Rbase : Type.
  Context {Reify_Ty : Reify (Ty 0)}.
  Context {Reify_expr_typ_func : Reify (expr (Ty 0) func)}.
  ...
  Definition reify_Proper_concl : Command (Proper_concl tsym func Rbase) :=
    CPattern (ls:=_::_::nil)
             (!!@Morphisms.Proper @ # @ ?0 @ ?1)
             (fun (a : function (CCall (reify_scheme (R (Ty 0) Rbase))))
                  (b : function (CCall (reify_scheme (expr (Ty 0) func)))) =>
                @mkProper tsym _ _ a b).

  Global Instance Reify_Proper_concl : Reify (Proper_concl tsym func Rbase) :=
